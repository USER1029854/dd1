...
| `DISC-20260912-012-mechanism-targeted-audit-queue.md` | **Deep, evidence-grounded, mechanism-first.** Studied **560 SlowMist incidents over 2 years**, classified scope+mechanism, tested the thesis, built a **mechanism library** (code shapes that drain), and **read deployed code** to separate guarded from exposed | evidence study + mechanism library + decisive-check reads + targeted queue | 305/560 on-chain; modal loss band \$100k–1M; mechanism A (spot-price, no-TWAP) is the modal hack; SOFA/Notional classes rare (4/6 in 2yr). Read Universe Finance (GUARDED-onlyEOA) & DefiEdge (GUARDED-deviation). 401-row queue ranked A2 oracle-collateral→A3 basis→A1 ALM→B/C, each with the exact function+guard to check. Honest: check validated, no live exposed instance confirmed (risk concentrates in unverified/A2/A3) |
| `DISC-20260912-011-onchain-vintage-and-forward-bespoke.md` | **Fresh discovery via on-chain reads, not listedAt.** Found the pre-AI seam looked exhausted by *listing date* (0 fresh), then read deployed **compiler vintage** on-chain to recover genuinely pre-AI-era bespoke projects that listedAt hid (TrueFi, CompliFi, Auctus, CoFiX, Tranche, DSU). Two tiers | discovery + on-chain vintage evidence | **Tier 1** 26 recovered pre-AI-vintage bespoke-unwatched (compiler 0.4–0.7; token-read caveat, mostly small — big ones already in DISC-010); **Tier 2** 101 forward 2023+ bespoke-unwatched (basis/delta-neutral vaults, the new risk-curator surface, stablecoin/RWA wrappers, abandoned CDP/bonding V1s). TVL-neutral, ⚠prior-incident flagged, generic forks dropped. Folded → exclusion set |
| `DISC-20260909-010-unwatched-bespoke-preai-audit-queue.md` | **The corrected audit queue (supersedes 008/009 ranking).** TVL is neutral, not a ranking axis (SOFA held ~$850k; its $10k loss described the finding, not the target). Ranked by **bespoke custom logic (non-fork) × unwatched × pre-AI-era** | ranked audit queue | 108 pre-2023 bespoke-unwatched financial primitives across the full TVL range, grouped by hand-rolled surface (options/synth/derivatives/lending-CDP/settlement/bridge/liq-mgr); a surface-diverse SOFA-band "start here" 14; previously-hacked remnants flagged; overlaps DISC-009 by design. Folded → exclusion set |
| `DISC-20260908-009-abandoned-preai-bespoke-seam.md` | **The unmined seam — 116 old, abandoned, hand-rolled protocols that still hold money.** Corrects DISC-008's logic: "echoes last week's hack" is a weak predictor (copycat window closes fast). Predictor is structural neglect — still funded + nobody paid to watch + **pre-AI-era bespoke (non-fork) code**. The SOFA shape | thesis + ranked queue | measured that **81% of the pre-2023 $50k+ band was already delivered**; dropping the floor to SOFA size (~$10k, SOFA was $10.8k) opened 257 new / 120 bespoke. Tier A=29 (bespoke credit, structured payoff, synthetics, parametric settlement, bridge verifiers), B=45, C=42. Folded in → 2,439 |
